...
public class EntityListener implements Listener {
    ...
    public void onEntityRemoveFromWorld(@NotNull EntityRemoveFromWorldEvent event) {
        SuperSeatBoi.getSeatManager().unregisterSeatEntity(event.getEntity().getUniqueId());
    }

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onEntityMount(@NotNull EntityMountEvent event) {
        Entity mountedEntity = event.getMount();
        SeatEntity seatEntity = SuperSeatBoi.getSeatManager().getSeatEntity(mountedEntity.getUniqueId());
        if (seatEntity != null) {
    ...
    public void onEntityDismount(@NotNull EntityDismountEvent event) {
        ...
        SeatEntity seatEntity = SuperSeatBoi.getSeatManager().getSeatEntity(dismountedEntity.getUniqueId());
        if (seatEntity != null) {
